...
* parse_helper:
*
* Counts the number of characters in a word, and then loops through the argument
* character by character, to create a pointer to an array of characters (a string)
* that is the first word in the argument. This will then be used by commmand_parser
* since an element in command** will point to this char* string.
*
* Parameter:
*
* - word: line passed in parser method but starting from the word that will be stored
* in returned char*
*
* Return:
*
...
for (char* p = line; *p; p++) {
if (*p == '&') {
	//if & has already appeared, *foreground = 0
	//more than one & is invalid, so return null
	if (*foreground == 0) {
		return NULL;
	} else {
...
//since wordCount incremented when there is a space in loop, if word ends with
//something else, inWord still is 1, wordCount must still be incremneted
if (inWord == 1) {
	wordCount++;
}
...
if (*p == '&' || *p == ' ') {
	...
	//if *p is a non-space char, and inWord == 0, it means *p is first letter of a word
        if (inWord == 0) {
 ...
 * command_show:
 *
 * Print the structure of a command array. Prints strings that command array holds and in
 * between each string is a '_'. _ indicates start and end of element, clearly delineates
 * word boundaries of command array and makes it easy to see if there is a space before the
 * last _ (which indicates that the element has a trailing space, and therefore invalid).
 *
 * Parameters:
...
//p is pointer to each element in command array
...
for (char** p = command; *p != NULL; p++) {
	char* pChar = (char*)*p;
	//while loop checks each char and prints if they are not ampersand, quote, space,
	//or newline
	while (*pChar) {
